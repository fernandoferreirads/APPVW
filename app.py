...
# ─── Conferência de Comissão — backend ────────────────────────────────────────

# Todos os produtos que um vendedor pode declarar numa conferência
PRODUTOS_CONFERENCIA = [
    "SPF BASICO", "SPF NORMAL", "SPF PLUS",
    "AP", "GAP", "FRANQUIA",
    "GE 1", "GE 2", "GE 4",
    "PROTEGE BAS 24", "PROTEGE BAS 36", "PROTEGE PLUS 24", "PROTEGE PLUS 36",
]


def _pontos_lista(produtos: list) -> float:
    return round(sum(PONTOS_PRODUTO.get(p, 0.0) for p in produtos), 2)


def _produtos_da_linha_reg(row) -> list:
    """Extrai lista de produtos a partir de uma linha do Sheets (Series ou dict)."""
    prods = []
    for campo in ["spf", "app", "gap", "franquia", "ge", "protege"]:
...
def _meses_disponiveis(n: int = 6) -> list:
    """Retorna os últimos n nomes de aba (mês atual + anteriores)."""
    now  = datetime.now()
    resultado = []
    for i in range(n):
        ...
        while m <= 0:
            m += 12
            y -= 1
        resultado.append(f"{MESES_PT[m]} {y}")
    return resultado


def ler_aba_vendedor(vendedor: str, aba: str,
                     spreadsheet_id: str, creds_path: str) -> pd.DataFrame:
    """Lê todos os registros de um vendedor na aba indicada."""
    service = get_sheets_service(creds_path)
    try:
    ...
    COLS = [
        "proposta", "equipe", "data", "cpf", "cliente",
        "valor_veiculo", "entrada", "financiado",
        "spf", "app", "gap", "franquia", "rev",
        "ge", "protege", "prazo", "taxa", "tipo", "sn",
        "peso", "vendedor", "retorno", "pontos",
        "sem_pts", "corr_ge", "corr_pts", "vz", "loja",
    ]
    padded = [r + [""] * max(0, 28 - len(r)) for r in rows]
    df = pd.DataFrame(padded, columns=COLS)
    df["proposta"] = df["proposta"].astype(str).str.strip()
    df["vendedor"] = df["vendedor"].astype(str).str.strip().str.upper()
    return df[df["vendedor"] == vendedor.strip().upper()].copy()


def conferir_producao(declarado: list, df_reg: pd.DataFrame) -> pd.DataFrame:
    """Cruza lista declarada pelo vendedor com os registros da planilha."""
    linhas = []
    for item in declarado:
        prop      = str(item["proposta"]).strip()
        prods_dec = item["produtos"]
        pts_dec   = _pontos_lista(prods_dec)

        match = df_reg[df_reg["proposta"] == prop]

        if match.empty:
            linhas.append({
                "Proposta":        prop,
                "Declarado":       ", ".join(prods_dec) or "—",
                "Registrado":      "Não encontrado",
                "Pts Declarado":   pts_dec,
                "Pts Registrado":  0.0,
                "Status":          "❌ Não encontrado",
            })
        else:
            row       = match.iloc[0]
            prods_reg = _produtos_da_linha_reg(row)
            try:
                pts_reg = float(str(row["pontos"]).replace(",", ".").strip())
            except Exception:
                pts_reg = _pontos_lista(prods_reg)

            ok = set(prods_dec) == set(prods_reg)
            linhas.append({
                "Proposta":        prop,
                "Declarado":       ", ".join(sorted(prods_dec)) or "—",
                "Registrado":      ", ".join(sorted(prods_reg)) or "—",
                "Pts Declarado":   pts_dec,
                "Pts Registrado":  pts_reg,
                "Status":          "✅ Confere" if ok else "⚠️ Divergência",
            })

    _cols = ["Proposta", "Declarado", "Registrado",
             "Pts Declarado", "Pts Registrado", "Status"]
    return pd.DataFrame(linhas, columns=_cols) if linhas else pd.DataFrame(columns=_cols)
...
<div class="section-title">
    ...
    <span>Conferência de Comissão</span>
</div>
<p style="color:#6b7280;font-size:0.875rem;margin:-0.5rem 0 1.25rem 32px;">
    Compare a produção declarada pelo vendedor com o que foi registrado na planilha de pagamento
</p>
""", unsafe_allow_html=True)

if "conf_itens" not in st.session_state:
    st.session_state["conf_itens"] = []

# ── Seleção de vendedor e mês ─────────────────────────────────────────────────
with st.container(border=True):
    col_cv, col_cm = st.columns(2)
    with col_cv:
        conf_vendedor = st.selectbox(
            "👤 Vendedor",
            options=sorted(VENDEDOR_EQUIPE.keys()),
            key="conf_vendedor",
        )
        _conf_equipe = lookup_vendedor(conf_vendedor)
        st.caption(f"🏢 Equipe: **{_conf_equipe}**")
    with col_cm:
        conf_mes = st.selectbox(
            "📅 Mês de Referência",
            options=_meses_disponiveis(),
            key="conf_mes",
        )

# ── Formulário de declaração ──────────────────────────────────────────────────
st.markdown("##### Adicionar contrato declarado")
with st.container(border=True):
    col_cp, col_cprod = st.columns([1, 3])
    with col_cp:
        conf_proposta = st.text_input(
            "Nº da Proposta",
            key="conf_proposta",
            placeholder="Ex: 14469703",
        )
    with col_cprod:
        conf_produtos = st.multiselect(
            "Produtos declarados",
            options=PRODUTOS_CONFERENCIA,
            key="conf_produtos",
            placeholder="Selecione os produtos deste contrato...",
        )
        _conf_pts = _pontos_lista(conf_produtos)
        st.caption(f"⭐ Pontos calculados: **{_conf_pts}**")

    col_cadd, _ = st.columns([1, 5])
    with col_cadd:
        conf_add = st.button("➕ Adicionar à lista", key="conf_add", use_container_width=True)

if conf_add:
    if not conf_proposta.strip():
        st.warning("⚠️ Informe o número da proposta.")
    else:
        # Evita duplicata de proposta na mesma conferência
        props_existentes = [it["proposta"] for it in st.session_state["conf_itens"]]
        if conf_proposta.strip() in props_existentes:
            st.warning("⚠️ Essa proposta já foi adicionada à lista.")
        else:
            st.session_state["conf_itens"].append({
                "proposta": conf_proposta.strip(),
                "produtos": list(conf_produtos),
            })
            st.rerun()

# ── Prévia da declaração ──────────────────────────────────────────────────────
if st.session_state["conf_itens"]:
    _ci = st.session_state["conf_itens"]
    st.info(f"**{len(_ci)} contrato(s)** na declaração de **{conf_vendedor}** — {conf_mes}")

    _ci_df = pd.DataFrame([
        {
            "Proposta": it["proposta"],
            "Produtos":  ", ".join(it["produtos"]) if it["produtos"] else "—",
            "Pontos":    _pontos_lista(it["produtos"]),
        }
        for it in _ci
    ])
    st.dataframe(_ci_df, use_container_width=True, hide_index=True)

    col_cf, col_clim = st.columns([4, 1])
    with col_clim:
        if st.button("🗑️ Limpar", key="conf_clear", use_container_width=True):
            st.session_state["conf_itens"] = []
            st.session_state.pop("conf_resultado", None)
            st.rerun()

    with col_cf:
        if not sheets_ok:
            st.warning("Configure o Google Sheets nas configurações para habilitar a conferência.")
        else:
            if st.button(
                f"🔍 Conferir produção de {conf_vendedor} — {conf_mes}",
                type="primary",
                key="conf_conferir",
                use_container_width=True,
            ):
                with st.spinner("Lendo planilha e comparando dados…"):
                    try:
                        _df_reg = ler_aba_vendedor(
                            conf_vendedor, conf_mes, spreadsheet_id, creds_path
                        )
                        _df_res = conferir_producao(st.session_state["conf_itens"], _df_reg)
                        st.session_state["conf_resultado"] = _df_res
                        st.session_state["conf_resultado_meta"] = {
                            "vendedor": conf_vendedor,
                            "equipe":   _conf_equipe,
                            "mes":      conf_mes,
                        }
                    except Exception as _e:
                        st.error(f"❌ Erro na conferência: {_e}")
                st.rerun()

# ── Resultado da Conferência ──────────────────────────────────────────────────
if st.session_state.get("conf_resultado") is not None:
    _df_res  = st.session_state["conf_resultado"]
    _cf_meta = st.session_state.get("conf_resultado_meta", {})

    st.markdown(
        f"### 📋 Resultado — {_cf_meta.get('vendedor','')} &nbsp;·&nbsp; "
        f"{_cf_meta.get('equipe','')} &nbsp;·&nbsp; {_cf_meta.get('mes','')}",
        unsafe_allow_html=True,
    )
    ...
    if _df_res.empty:
        ...
        _n_ok  = (_df_res["Status"] == "✅ Confere").sum()
        _n_div = (_df_res["Status"] == "⚠️ Divergência").sum()
        _n_nao = (_df_res["Status"] == "❌ Não encontrado").sum()
        _pts_d = _df_res["Pts Declarado"].sum()
        _pts_r = _df_res["Pts Registrado"].sum()

        # Métricas resumo
        _m1, _m2, _m3, _m4, _m5 = st.columns(5)
        _m1.metric("Contratos", len(_df_res))
        _m2.metric("✅ Conferem", int(_n_ok))
        _m3.metric("⚠️ Divergências", int(_n_div))
        _m4.metric("❌ Não encontrados", int(_n_nao))
        _m5.metric(
            "Pontos decl. / regist.",
            f"{_pts_d:.2f} / {_pts_r:.2f}",
            delta=round(_pts_d - _pts_r, 2),
            delta_color="inverse",
        )
        ...
        st.dataframe(
            _df_res.style.map(_cor_status, subset=["Status"]),
            use_container_width=True,
            hide_index=True,
        )

        if _n_div == 0 and _n_nao == 0:
            st.success("🎉 Produção 100% conferida! Nenhuma divergência encontrada.")
            st.balloons()
        else:
            st.warning(
                f"⚠️ **{int(_n_div + _n_nao)} item(ns)** com divergência ou não encontrado(s). "
                "Revisar antes de confirmar a comissão."
            )

        col_res_lim, _ = st.columns([1, 4])
        with col_res_lim:
            if st.button("🗑️ Limpar resultado", key="conf_clear_res", use_container_width=True):
                st.session_state.pop("conf_resultado", None)
                st.session_state.pop("conf_resultado_meta", None)
                st.session_state["conf_itens"] = []
                st.rerun()
